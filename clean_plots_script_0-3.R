...
datan$rf_star <- boxcox(datan$rf_mean, l_rf)

m3 <- glm(liq ~ qc1_star + CSR_star + rf_star, family = "binomial", data = datan)
bees <- coefficients(m3)
...
for (i in 1:length(nz$qc1)){
  nz$p[i] <- 1/(1+exp(-(b0+b1*boxcox(nz$qc1,l_qc1)[i]+b2*boxcox(nz$CSR,l_csr)[i]+b3*boxcox(nz$rf, l_rf)[i])))
}
