...
draws <- read.csv("/Users/JSchmidt/OneDrive - Yeh & Associates/Documents/Personal Library/Paper/fmodel1.2p10.csv") #make sure you have the right stanfit
b0 <- draws$b0
b1 <- draws$b1
b2 <- draws$b2
...
ggplot(data = as.data.frame(b0), aes(x = b0)) + geom_histogram(aes(y=..density..), color = "black", fill = "white") + theme_classic() + geom_density() +
  geom_vline(aes(xintercept = quantile(b0, 0.5)), linetype = "dashed") + geom_vline(aes(xintercept = quantile(b0, 0.05)), linetype = "dashed") + 
  geom_vline(aes(xintercept = quantile(b0, 0.95)), linetype = "dashed") + ggtitle(paste("Mean = ", round(mean(b0),2),"SD = ", round(sd(b0),3))) +
